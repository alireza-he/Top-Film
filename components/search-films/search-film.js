...
function fetchFilms(value, callback) {
    ...
    function fetch() {
        axios.get("/api/admin/home/banner", {params: {text: value, type: "search"}}).then(res => {
            console.log(res)
            callback(res.data);
        }).catch(err => {
            console.log(err)
...
class SearchFilm extends React.Component {
    ...
    render() {
        ...
        const options = banner !== null && !status ?
            (<Option value={banner._id} key={banner._id}>{banner.name}</Option>)
            :
            (this.state.data.map(d => <Option value={d._id} key={d._id}>{d.name}</Option>))
        return (
            <Select
                showSearch
