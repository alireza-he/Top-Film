...
const Loader = ({isFirstLoading, setLoader}) => {
    const {isAuthenticated, fetchCompleted} = useContext(AuthContext)

    useEffect(() => {
        if (isAuthenticated() && isFirstLoading || !isAuthenticated() && isFirstLoading && fetchCompleted) {
            setLoader(false)
        }
    })
    ...
    return (
        <main>
            <div className="dank-ass-loader">
                ...
                {isFirstLoading ? <p className="text-white rtl mt-3 h4">به تاپ فیلم خوش آمدید</p> : ''}
            </div>
        </main>
    );
};
